...
vignette: >
  %\VignetteIndexEntry{Cumulative water deficit example}
...
df |> 
  ...
  geom_line() +
  labs(x = "Date", y = "Snow mass equivalent (mm)")
```
...
Plot any variable contained in the original data frame as a "superimposed epoch analysis", that is with data from multiple events (indicated by index `iinst`) versus the day into event (`dday`).
```{r}
out_cwd$df |> 
  ggplot(aes(dday, et, group = iinst)) +
  geom_line(alpha = 0.5) +
  theme_classic()
```

The instances (events) are available in summarised form:
```{r}
out_cwd$inst
```

You can for example plot the distribution of event (instance) length (here in days):
```{r}
out_cwd$inst |> 
  ggplot(aes(len, after_stat(count))) +
  geom_histogram(color = "black", fill = "grey60") +
  labs(x = "Event length (d)")
```
...
vals <- out_cwd$inst %>%
  ...
  summarise(deficit = max(max_deficit, na.rm = TRUE)) %>%
  pull(deficit)
...
ggplot() +
  geom_histogram(
    data = out_cwd$inst,
    aes(x = max_deficit, y = after_stat(density)),
    color = "black",
    position="identity",
    bins = 6
    ) +
