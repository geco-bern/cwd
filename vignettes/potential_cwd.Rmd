...
2) `threshold_terminate_absolute`: threshold determining end of event as `thresh_terminate` but in absolute terms (in mm d-1).
...
out_cwd <- cwd(
 ...
 #thresh_terminate = 2,
  #doy_reset = 999,
  thresh_terminate_absolute = 100.0,
  thresh_drop = 0.0
  )
```
...
ggplot() +
  geom_rect(
    data = out_cwd$inst,
    aes(xmin = date_start, xmax = date_end, ymin = 0, ymax = max( out_cwd$df$deficit)), #reset to 0 for ylim
    fill = rgb(0,0,0,0.3),
    color = NA) +
  geom_line(data  =  out_cwd$df, aes(TIMESTAMP, deficit), color = "tomato") +
  theme_classic() +
  ylim(0, max(out_cwd$df$deficit)) + #reset to 0 for ylim
  labs(
    x = "Date", 
