...
df_CH_LAE <- read_csv(paste0(here(), "/inst/extdata/FLX_CH-Lae_FLUXDATAKIT_FULLSET_DD_2004_2014_2-3.csv")) |>
  select(TIMESTAMP, P_F, TA_F_MDS, PA_F, LE_F_MDS, NETRAD) |>
  filter(year(TIMESTAMP) > 2004)

# get fluxnet data for FR-Pue site
df_FR_PUE <- read_csv(paste0(here(), "/inst/extdata/FLX_FR-Pue_FLUXDATAKIT_FULLSET_DD_2000_2014_2-3.csv")) |>
...
# saveRDS(df_FR_PUE, file = paste0(here(), "/data/df_fr-pue.rds"))

# usethis::use_data(df_CH_LAE, internal = TRUE)
# saveRDS(df_CH_LAE, file = paste0(here(), "/data/df_ch-lae.rds"))
